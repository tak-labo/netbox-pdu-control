...
### Added
- `docs/structure.html` + `docs/structure.json` — a single self-contained,
  dependency-free HTML page with an interactive node graph of the plugin's
  module structure and data model (linked from `docs/design.md`).
...
- `ManagedPDUForm`: expose `config_backup_enabled` (add/edit form and
  ...
  previously only settable via the Django shell
