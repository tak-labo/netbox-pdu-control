...
class ManagedPDUTable(NetBoxTable):
    ...
    last_metrics_fetched = tables.DateTimeColumn(verbose_name="Last Metrics Fetched")
...
<form method="post" action="{% url 'plugins:netbox_pdu_control:pduoutlet_power_on' pk=record.pk %}" style="display:inline">
  {% csrf_token %}
  <button type="submit" class="btn btn-sm btn-success" title="Power ON">
    <i class="mdi mdi-power"></i>
  </button>
</form>
<form method="post" action="{% url 'plugins:netbox_pdu_control:pduoutlet_power_off' pk=record.pk %}" style="display:inline">
  {% csrf_token %}
  <button type="submit" class="btn btn-sm btn-danger" title="Power OFF">
    <i class="mdi mdi-power-off"></i>
  </button>
</form>
<form method="post" action="{% url 'plugins:netbox_pdu_control:pduoutlet_power_cycle' pk=record.pk %}" style="display:inline">
  {% csrf_token %}
  <button type="submit" class="btn btn-sm btn-warning" title="Power Cycle">
    <i class="mdi mdi-restart"></i>
  </button>
</form>
<form method="post" action="{% url 'plugins:netbox_pdu_control:pduoutlet_sync' pk=record.pk %}" style="display:inline">
  {% csrf_token %}
  <button type="submit" class="btn btn-sm btn-outline-primary" title="Sync this outlet">
    <i class="mdi mdi-refresh"></i>
  </button>
</form>
<form method="post" action="{% url 'plugins:netbox_pdu_control:pduoutlet_push_name' pk=record.pk %}" style="display:inline">
  {% csrf_token %}
...
INLET_SYNC_BUTTON = """
{% if perms.netbox_pdu_control.change_managedpdu %}
<form method="post" action="{% url 'plugins:netbox_pdu_control:pduinlet_sync' pk=record.pk %}" style="display:inline">
  {% csrf_token %}
  <button type="submit" class="btn btn-sm btn-outline-primary" title="Sync this inlet">
    <i class="mdi mdi-refresh"></i>
  </button>
</form>
<form method="post" action="{% url 'plugins:netbox_pdu_control:pduinlet_push_name' pk=record.pk %}" style="display:inline">
  {% csrf_token %}
  <button type="submit" class="btn btn-sm btn-outline-secondary" title="Push name to PDU">
    <i class="mdi mdi-upload"></i>
  </button>
