...
class ManagedPDUFilter(NetBoxModelFilter):
    sync_status: Annotated["SyncStatusEnum", strawberry.lazy("netbox_pdu_control.graphql.enums")] | None = (
...
class PDUOutletFilter(NetBoxModelFilter):
    managed_pdu: Annotated["ManagedPDUFilter", strawberry.lazy("netbox_pdu_control.graphql.filters")] | None = (
        strawberry_django.filter_field()
    )
    managed_pdu_id: ID | None = strawberry_django.filter_field()
    status: Annotated["OutletStatusEnum", strawberry.lazy("netbox_pdu_control.graphql.enums")] | None = (
        strawberry_django.filter_field()
    )
