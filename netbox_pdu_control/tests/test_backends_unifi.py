...
from netbox_pdu_control.backends.unifi import UniFiPDUClient
...
def _make_client(username="admin", password="secret", base_url="https://unifi.example.com", **kwargs):
    ...
    with patch("netbox_pdu_control.backends.unifi.requests.Session") as mock_session_cls:
...
class TestSetOutletPowerState(unittest.TestCase):
    ...
    def test_cycle_sends_off_then_on(self):
        ...
        with patch("netbox_pdu_control.backends.unifi.time.sleep"):
    ...
    def test_outlet_index_is_1based(self):
        ...
        put_payload = self.client._put.call_args[0][1]
        modified_indices = [o["index"] for o in put_payload["outlet_overrides"] if o["relay_state"]]
...
class TestGetOutletPowerStateByIndex(unittest.TestCase):
    def _make_device(self, relay_states):
        return {
            "outlet_table": [{"index": i + 1, "relay_state": state} for i, state in enumerate(relay_states)],
        }
