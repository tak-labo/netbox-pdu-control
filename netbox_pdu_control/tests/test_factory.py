...
from netbox_pdu_control.backends import _VENDOR_BACKENDS, get_pdu_client
...
def _mock_managed_pdu(
    vendor, api_url="https://pdu.example.com", api_username="admin", api_password="secret", verify_ssl=True
):
...
class TestGetPduClientFactory(unittest.TestCase):
    ...
    def test_ubiquiti_returns_unifi_client(self):
        ...
        with patch("netbox_pdu_control.backends.unifi.requests.Session"):
            client = get_pdu_client(managed_pdu)
    ...
    def test_all_registered_vendors_can_be_instantiated(self):
        ...
        for vendor_key in _VENDOR_BACKENDS:
            ...
            with (
                ...
                patch("netbox_pdu_control.backends.unifi.requests.Session"),
            ):
                client = get_pdu_client(managed_pdu)
