...
class UniFiPDUClient(BasePDUClient):
    ...
    def __init__(
        self, base_url: str, username: str, password: str, verify_ssl: bool = True, managed_pdu=None, **kwargs
    ):
    ...
    def _login(self) -> None:
        ...
        raise PDUClientError("Failed to authenticate with UniFi controller. Check api_url, username, and password.")
    ...
    def get_single_outlet_data(self, outlet_index: int) -> dict:
        ...
        if outlet_index >= len(all_outlets):
            raise PDUClientError(f"Outlet index {outlet_index} out of range (total: {len(all_outlets)})")
    ...
    def get_single_inlet_data(self, inlet_index: int) -> dict:
        ...
        if inlet_index >= len(inlets):
            raise PDUClientError(f"Inlet index {inlet_index} out of range (total: {len(inlets)})")
