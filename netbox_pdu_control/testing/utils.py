"""
Utility functions for testing NetBox PDU Plugin.
...
def create_test_user(
    ...
    Example:
        ...
        ...     permissions=['netbox_pdu_control.view_pdu']
        ... )
    """
    if username is None:
...
def get_deletable_objects(instances):
    ...
    Example:
        >>> objs = Pdu.objects.filter(name__startswith='test')
        >>> deletions = get_deletable_objects(objs)
        >>> # deletions = {'Pdu': 3, 'RelatedModel': 5}
    """
...
def assert_object_changes(test_case, instance, action: str, user: User | None = None, message: str | None = None):
    ...
    Example:
        ...
        >>> obj = Pdu.objects.create(name='Test')
